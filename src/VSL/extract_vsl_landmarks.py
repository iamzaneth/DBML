...
INTERIM_BASE_DIR = PROJECT_ROOT / "data" / "interim" / "VSL_cropped"
OUTPUT_BASE_DIR = PROJECT_ROOT / "data" / "processed" / "v2" / "VSL"
...
PREVIEW_BASE_DIR = PROJECT_ROOT / "data" / "processed" / "v2" / "VSL_preview"
PREVIEW_FPS = 10.0
...
# - WORKER_ID = 1..10: chia đều các label VSL đã sort theo tên folder.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract VSL MediaPipe landmarks with NumPy keypoint preprocessing."
    )
    ...
    parser.add_argument(
        "--worker-id",
        type=int,
        help="Worker id. 0 means full run; 1..10 split sorted label folders evenly.",
    )
...
def main():
    ...
    print("=" * 80)
    print("EXTRACT-ONLY OPTIMIZED VSL HAND + POSE + FACE-BLENDSHAPE LANDMARKS")
    print("=" * 80)

    if WORKER_ID < 0 or WORKER_ID > NUM_WORKERS:
        raise ValueError(f"WORKER_ID must be 0 or 1-{NUM_WORKERS}. 0 runs full.")

    run_all_labels = WORKER_ID == 0
    if run_all_labels:
    ...
    if not INTERIM_BASE_DIR.exists():
        ...
        print("Bạn cần có cấu trúc: data/interim/VSL_cropped/<label>/*.mp4")
        return
    ...
    print(f"[WORKER CONFIG] NUM_WORKERS={NUM_WORKERS}")
    ...
    if not sample_mode and not run_all_labels:
        labels_per_worker = max(1, (len(label_dirs) + NUM_WORKERS - 1) // NUM_WORKERS)
        start_idx = (WORKER_ID - 1) * labels_per_worker
        end_idx = min(start_idx + labels_per_worker, len(label_dirs))
        print(f"[WORKER] Worker {WORKER_ID}/{NUM_WORKERS}: labels index {start_idx} to {end_idx - 1}")
